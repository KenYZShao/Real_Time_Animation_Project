...
uniform vec3 eyePosition;//cameraPos;


vec3 calcReflection(){
	
	//float ratio = 1.00 / 1.00;//Shao edited 
	vec3 I = normalize(FragPos - eyePosition);
	vec3 R = reflect(I, normalize(Normal));//Original
    //vec3 R = reflect(I, normalize(Normal),ratio);//Shao edited from LearnOpenGL's Shader
	vec3 outcolor = vec3(texture(skybox, R).rgb);//Original Code
	//outcolor = vec4(texture(skybox, R).rgb,1.0);//Shao edited from LearnOpenGL's Shader
...
//const float ratio = 1.00/1.52;//for glass
const float ratio = 1.00/2.24;//for diamond
//const float ratio = 1.00/1.309;//for ice
//const float ratio = 1.00/1.33;//for water
//const float ratio = 1.00/1.00;//for air
//const float ratio = 1.00/2.24;
...
void main()
{
	...
	colour = vec4(calcReflection(), 1.0);//Previously this row was comment
	//colour = vec4(calcRefraction(), 1.0);
	//colour = vec4(calcFresnel(), 1.0);
	//colour = vec4(calcDispersion(),1.0);
	//colour = vec4(result, 1.0);//Original
}